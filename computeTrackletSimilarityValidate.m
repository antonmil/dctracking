...
featnormfile=sprintf('tmp/linked/simMat-%s-featnorm.mat',setting);
load(featnormfile);
...
wt.dist=1;
wt.scal=mean([allWT(:).scal]);
wt.appe=mean([allWT(:).appe]);
wt.gaps=mean([allWT(:).gaps]);
...
while loopscale>mean(allScales)
% while min(C(:))<mean(allThetas)
% while min(C(:))<-0.196464
% while length(remids)<round(N/2) %&& firstMOTA<1.2*metrics2d(12)
    
    [mind, bestpair]=min(C(:));
    if isinf(mind), break; end
    allmind=[allmind mind];
    [b1, b2]=ind2sub(size(C),bestpair);
...
if savesim
    resfiledir=sprintf('tmp/linked/simres/%s/',setting);
