function [metrics2d metrics3d stateInfo] = run_mytracker( scen, options )
%TRACKER Summary of this function goes here
%   Detailed explanation goes here

%% Load example video
% if nargin == 0
%     avi = mmreader('ants.avi').read;
% end
%% Configuration
...
global opt sceneInfo detections scenario gtInfo
...
tracker = com.multiplehypothesis.simpletracker.tracker.MTracker(...
    opt.maxNumLeaves,... % int maxNumLeaves
    opt.maxDepth,... % int maxDepth
    opt.timeUndetected,... % int timeUndetected
    opt.bestK,... % int bestK
    opt.probUndetected,... % double probUndetected
    opt.probNewTarget,... % double probNewTarget
    opt.probFalseAlarm,... % double probFalseAlarm
    opt.gateSize ... % double gateSize
    );
...
%     im=double(imread([sceneInfo.imgFolder sprintf(sceneInfo.imgFileFormat,sceneInfo.frameNums(i))]))/255;
%     imshow(im);
%     hold on

    dets=[detections(i).xp' detections(i).yp'];
%     for n=1:size(dets,1)
%         plot(dets(n,1),dets(n,2), 'r*')
%     end
%     detections
