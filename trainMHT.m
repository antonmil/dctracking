function trainMHT(jobname,jobid,maxexper)
...
addpath(genpath('external/mhttracking'))
...
if exist(resultsfile,'file')
  ...
  % otherwise, randomize and write out new ini file
	  ...
	  if jobid <= maxexper/2
		  params = 2*rand(1,length(params)) .* rnmeans; % uniform [0, 2*max] or [2*max, 0]
	  else
...
opt.probNewTarget=max(0,opt.probNewTarget);
opt.probFalseAlarm=max(0,opt.probFalseAlarm);
opt.gateSize=max(0,opt.gateSize);

	  % write out new opt file
	  status = writeMHTOptions(opt,conffile);
