function trainDP(jobname,jobid,maxexper)

%% determine paths for config, logs, etc...
addpath(genpath('../motutils/'));
addpath(genpath('external/ksptracking'))
format compact
...
if bestexper==1
  ...
  if rjobs<=0
	...
	% 
	submitstr=sprintf('!ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no moby  \"cd research/projects/dctracking; sh submitKSPTrain.sh %s\"',newSetting)
	fprintf('submit: %s\n',newSetting)
  	eval(submitstr);	
  else
